...
def build_vqa_model(ntoken, num_ans_candidates, num_hid=1024, v_dim=2048):
    from attention import Attention, NewAttention
    from language_model import WordEmbedding, QuestionEmbedding
    from classifier import SimpleClassifier
    from fc import FCNet
    from base_model import BaseModel
    w_emb = WordEmbedding(ntoken, 300, 0.0)
...
def get_vqg_input(questions, answers, tokenizer, max_len, include_action=0, actions=None):
    ...
    for b in range(batch_size):
        ...
        if include_action and turn>0:
            tokens += ['[SEP]'] + tokenizer.tokenize(actions[b][turn-1])
...
# for BUTD-vqa
def get_vqa_input(questions, answers, dictionary, max_length=50):
    ...
    for b in range(batch_size):
        ...
        if turn==0:
            tokens = questions[b][0]
        else:
            tokens = questions[b][turn-1] + answers[b][turn-1] + ' ' + questions[b][turn]
        
        tokens = tokens.replace(' - ', '-')
        tokens = dictionary.tokenize(tokens, False)
        
        tokens = tokens[:max_length]
        ques_len.append(len(tokens))
        tokens = [dictionary.padding_idx] * (max_length - len(tokens)) + tokens
        input_ids.append(tokens)

    input_ids = torch.tensor(input_ids)
    ques_len = torch.tensor(ques_len)
    return input_ids, ques_len
...
def solve(args, id_list):
    ...
    sys.path.append(args.butd_path)
    ...
    dictionary = Dictionary.load_from_file(args.dataroot + '/' + 'dictionary.pkl') 
    vqa = build_vqa_model(dictionary.ntoken, len(ans2label)).to(args.device)
    ...
    for i, batch in enumerate(eval_dataloader):
        ...
        for t in range(args.max_turn):
            ...
            if args.mode in ['vqg_vqa_guesser']:
                ...
                q_input_ids, q_attention_mask, q_token_type_ids = get_vqg_input(questions, answers, vqg_tokenizer, args.max_hist_len, include_action=args.include_action, actions=actions)
                ...
                with torch.no_grad():
                    ...
                    output_sequences = vqg.generate(
                        ...
                        bos_token_id=vqg_tokenizer.cls_token_id,
                        eos_token_id=vqg_tokenizer.sep_token_id,
                        pad_token_id=vqg_tokenizer.pad_token_id
                    )
                ...
                for b in range(batch_size):
                    ...
                    if args.reverse_target:
                        x.reverse()
                    ques = vqg_tokenizer.decode(x, skip_special_tokens=True).encode('ascii', 'ignore').decode('ascii')
            ...
            if args.mode in ['vqg_vqa_guesser', 'golden_vqa_guesser']:
                ...
                a_input_ids, _ = get_vqa_input(questions, answers, dictionary)
                a_input_ids = a_input_ids.to(args.device)
                with torch.no_grad():
                    # logits = vqa(a_feat, a_boxes, a_sent, visual_attention_mask=a_visual_attention_mask)
                    logits = vqa(a_feat, None, a_input_ids, None)
